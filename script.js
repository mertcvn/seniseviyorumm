...
// Yardımcı: Rastgele benzersiz seç
function getUniqueRandomActivity(pool, usedList) {
    if (usedList.length >= pool.length) {
        usedList = []; // Sıfırla
    }
    let available = pool.filter(a => !usedList.includes(a));
...
function generateSurprise() {
    ...
    const cards = ['cardRomantic', 'cardFun', 'cardMemory'];
    cards.forEach(id => {
        const card = document.getElementById(id);
        card.classList.add('revealed');
    });

    // Favori butonu ekleyelim (kart arkasına dinamik olarak)
    document.querySelectorAll('.activity-card .card-back').forEach(back => {
        if (!back.querySelector('.fav-btn')) {
            ...
            favBtn.style.cssText = 'margin-top:10px; background:#ff4d6d; color:white; border:none; padding:5px 12px; border-radius:15px; cursor:pointer; font-size:0.8rem;';
...
function renderFavorites() {
    ...
    if (favorites.length === 0) {
        ...
        favorites.forEach((fav, index) => {
            ...
            li.innerHTML = `
                <span>${fav}</span>
                <button class="fav-remove-btn" data-index="${index}">🗑️</button>
            `;
...
// Günün rengini ayarla
function setDailyColor() {
    const today = new Date();
    const dayIndex = today.getDate() % colorsOfDay.length;
    const color = colorsOfDay[dayIndex];
...
// Günün şarkısını ayarla
function setDailySong() {
    const today = new Date();
    const dayIndex = today.getDate() % songsOfDay.length;
    const song = songsOfDay[dayIndex];
    document.getElementById('songTitle').textContent = song.title;
    document.getElementById('songArtist').textContent = song.artist;
    const player = document.getElementById('youtubePlayer');
    player.src = `https://www.youtube.com/embed/${song.videoId}?autoplay=1&controls=0&loop=1&playlist=${song.videoId}&mute=0&enablejsapi=1`;
}
...
function toggleMusic() {
    ...
    if (player.src.includes('autoplay=1')) {
        ...
        document.getElementById('toggleMusic').textContent = '🔇 Ses Aç';
    } else {
        player.src = player.src.replace('autoplay=0', 'autoplay=1');
        document.getElementById('toggleMusic').textContent = '🔇 Ses Kapat';
    }
}
...
function setDailyQuote() {
    const today = new Date();
    const index = today.getDate() % loveQuotes.length;
    document.getElementById('loveQuote').textContent = loveQuotes[index];
}

// Birlikte geçen gün sayısı (başlangıç: 3 hafta önce)
...
// Tarih inputunun bugünün tarihini varsayılan olarak ayarlama
